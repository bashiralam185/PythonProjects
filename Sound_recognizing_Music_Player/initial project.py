...
def about():
    ...
    if hour>=0 and hour<=12:
        ...
        morning("Good Morning. Thanks for using Raamzz player. This player is made by Python coding. The libraries that are used in the coding are tkinter, pygame, datetime, os and PIL. Creater of this music player is Bashir Alam")
...
def play1():
    ...
    try:  # this oortion checks for the pause.. if it is paused the code will executed and if not it will go to the except
        ...
        try:
            ...
            selected_song= int(selected_song[index])
...
def next_song():
    try:
        ...
        def indexerror(text):
            engine.say(text)
            engine.runAndWait()
        indexerror("Dear user, there are no songs to play next. add more songs to play list or next back to listen to the previous songs")
...
nextbtn=ttk.Button(player, text="Next song", command=next_song)
nextbtn.place(x=300, y=120)
